...
function Navbar() {
  ...
  const [countItems, setCountItems] = useState(countItemsCart())

  useEffect(() => {
    setCountItems(countItemsCart);
  });
  ...
  return (
    <Menu inverted color='purple' stackable size='massive' style={{borderRadius: 0}}>
      <Container>
        <Menu.Item header>Intershop-UI</Menu.Item>
        ...
        <Menu.Menu position='right'>
          ...
          <Menu.Item as={Link} to="/cartpage" style={userPageStyle()}>
            <Icon name='cart' />
            <Label color='red'>
               {countItemsCart()}
            </Label>
